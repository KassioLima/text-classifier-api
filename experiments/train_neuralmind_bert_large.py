...
def train_one_task(
    ...
    original_eval_rows = len(eval_df)

    train_target_counts = train_df["target"].value_counts().sort_index()
    kept_targets = train_target_counts[train_target_counts >= min_train_samples].index.tolist()
    dropped_targets = train_target_counts[train_target_counts < min_train_samples].to_dict()
    if not kept_targets:
        raise ValueError(
            f"Nenhuma classe de {cfg.task_name} tem pelo menos {min_train_samples} exemplos no treino."
        )

    train_df = train_df[train_df["target"].isin(kept_targets)].copy()
    eval_df = eval_df[eval_df["target"].isin(kept_targets)].copy()
    if eval_df.empty:
        raise ValueError(
            f"Nenhuma linha de avaliacao sobrou para {cfg.task_name} com min_train_samples={min_train_samples}."
        )
    ...
    metrics = {
        ...
        "original_eval_rows": int(original_eval_rows),
        "train_rows": int(len(train_df)),
        "eval_rows": int(len(eval_df)),
        "num_labels": int(len(label_encoder.classes_)),
        "kept_classes": label_encoder.classes_.tolist(),
        "dropped_classes": {str(k): int(v) for k, v in dropped_targets.items()},
...
def parse_args():
    ...
    parser.add_argument("--min-train-samples", type=int, default=1, help="Remove classes com menos exemplos que este valor no treino.")
    parser.add_argument(
        "--min-train-samples-by-task",
        nargs="*",
        default=[],
        metavar="TASK=MIN",
...
def parse_min_samples_by_task(values: list[str]) -> dict[str, int]:
    result = {}
    for value in values:
        if "=" not in value:
            raise ValueError(f"Formato invalido para --min-train-samples-by-task: {value}. Use TASK=MIN.")
        task, min_value = value.split("=", 1)
        task = task.strip()
        if task not in TASKS:
            raise ValueError(f"Tarefa invalida em --min-train-samples-by-task: {task}. Opcoes: {sorted(TASKS)}")
        try:
            parsed_min = int(min_value)
        except ValueError as exc:
            raise ValueError(f"Minimo invalido para {task}: {min_value}") from exc
        if parsed_min < 1:
            raise ValueError(f"Minimo invalido para {task}: {parsed_min}. Use valor >= 1.")
        result[task] = parsed_min
    return result
...
def main():
    args = parse_args()
    set_seed(args.seed)
    min_samples_by_task = parse_min_samples_by_task(args.min_train_samples_by_task)
    ...
    for task in args.tasks:
        ...
        print(f"[{task}] min_train_samples: {min_train_samples}")
